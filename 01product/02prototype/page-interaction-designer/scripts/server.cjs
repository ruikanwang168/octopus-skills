...
const SESSION_DIR = process.env.BRAINSTORM_DIR || '/tmp/page-interaction-designer';
const CONTENT_DIR = path.join(SESSION_DIR, 'content');
const STATE_DIR = path.join(SESSION_DIR, 'state');
const COMPANION_NAME = 'Page Interaction Designer';
...
<html>
<head><meta charset="utf-8"><title>Page Interaction Designer</title>
<style>
...
.brand-mark { display: inline-flex; align-items: center; justify-content: center; width: 1.25rem; height: 1.25rem; border-radius: 0.35rem; background: #1d1d1f; color: #fff; font-weight: 700; font-size: 0.72rem; letter-spacing: 0; flex-shrink: 0; }
</style>
</head>
<body><!-- BRANDING --><h1>Page Interaction Designer</h1>
<p>Waiting for the agent to push an interaction screen...</p></body></html>`);
}
...
function brandMarkup() {
  const text = escapeHtmlText(COMPANION_NAME);
  return '<div class="brand"><a href="#" aria-label="' + text + '"><span class="brand-mark">PI</span><span class="brand-copy">' + text + '</span></a></div>';
}
...
function handleMessage(text) {
  ...
  if (event && (event.choice || event.value)) {
